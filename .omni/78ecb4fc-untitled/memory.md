...
- 線上投票提交草稿在 `personal-auto/online-voting-submission.md`；2026-07-27 依 Sofia 回饋「不夠吸睛」全面重製為 v2 三版：v2-A 3D 吉祥物、v2-B 普普爆點海報、v2-C 電影感實景（檔名 `vote-submit-v2-*-768x514.png`），底圖用 gpt-image-1 生成、繁中文字以 Noto Sans TC（下載於 `.omni/tmp/fonts/`）程式合成避免 AI 錯字；建議主投 v2-A，作品介紹改為 73/76/73 字三版（含投票 CTA），v1 圖與文案封存於草稿文末。2026-07-27 已獨立驗證 Cloudflare Pages 作品展示連結 `https://from-carbon-metro-vote.pages.dev/` 可開、可手機 UA 取得 200、互動 smoke test 通過；GitHub Pages production 仍是舊版，`from-carbon-to-metro` 本地分支 ahead 6，且 GitHub API 認證回 401。
...
---
_Last system refresh: 2026-07-27 05:56 UTC_
